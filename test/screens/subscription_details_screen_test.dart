...
void main() {
  testWidgets(
    'subscription details screen renders active status, privileges and cancellation options',
    (tester) async {
      ...
      expect(
        find.text('Method 2: Secure AppsPro Cancellation'),
        findsOneWidget,
      );
      ...
      expect(
        find.text('Do you really want to unsubscribe from Premium?'),
        findsOneWidget,
      );
  ...
  testWidgets(
    'subscription details screen renders inactive banner when not subscribed',
    (tester) async {
      final dio = DioClient.create(baseUrl: 'https://example.com');
      final bdApiClient = BdAppsApiClient(dio);
      final bdAppsService = BdAppsService(apiClient: bdApiClient);
      ...
      expect(
        find.text('Your Subscription is Currently Inactive'),
        findsOneWidget,
      );
