...
document.addEventListener('DOMContentLoaded', () => {
  ...
  dose1?.addEventListener('click', () => {
    ...
    if (dosePill1) {
      ...
      dosePill1.className = dose1Taken ? 'med-action-pill med-action-taken' : 'med-action-pill med-action-pink';
    }
  });
  ...
  const genericDatabase = {
    napa: {
      brand: 'Napa 500mg Tablet', manufacturer: 'Beximco Pharmaceuticals Ltd.',
      price: 'MRP: ৳1.20 / tab', generic: 'Paracetamol 500mg',
      alternatives: [
    ...
    seclo: {
      brand: 'Seclo 20mg Capsule', manufacturer: 'Square Pharmaceuticals PLC',
      price: 'MRP: ৳7.00 / cap', generic: 'Omeprazole 20mg',
      alternatives: [
    ...
    montene: {
      brand: 'Montene 10mg Tablet', manufacturer: 'Incepta Pharmaceuticals Ltd.',
      price: 'MRP: ৳16.00 / tab', generic: 'Montelukast 10mg',
      alternatives: [
    ...
    sergel: {
      brand: 'Sergel 20mg Capsule', manufacturer: 'Healthcare Pharmaceuticals Ltd.',
      price: 'MRP: ৳10.00 / cap', generic: 'Esomeprazole 20mg',
      alternatives: [
  ...
  document.querySelectorAll('.med-search-chip').forEach((chip) => {
    chip.addEventListener('click', () => {
      document.querySelectorAll('.med-search-chip').forEach((c) => c.classList.remove('active'));
  ...
  logDoseBtn?.addEventListener('click', () => {
    ...
    if (streakCountDisplay) streakCountDisplay.textContent = `${currentStreak} Day Streak 🔥`;
    logDoseBtn.classList.toggle('logged', loggedToday);
    logDoseBtn.querySelector('span').textContent = loggedToday ? '✓ Dose Logged for Today' : '+ Log Afternoon Dose';
  });
  ...
  copySummaryBtn?.addEventListener('click', () => {
    ...
    if (label) {
      ...
      setTimeout(() => { label.textContent = '📋 Copy Summary to Clipboard'; }, 2500);
    }
  });
  ...
  function updateCalculator() {
    ...
    if (medsValueBadge) medsValueBadge.textContent = `${dailyMeds} ${dailyMeds === 1 ? 'medication' : 'medications'}`;
    if (spendValueBadge) spendValueBadge.textContent = `৳${monthlySpend.toLocaleString('en-IN')} / month`;
    ...
    tweenText(annualDosesVal, annualDoses, (v) => Math.round(v).toLocaleString('en-IN'));
    tweenText(missedPreventedVal, missedPrevented, (v) => `${Math.round(v).toLocaleString('en-IN')} doses`);
    tweenText(genericSavingsVal, annualSavings, (v) => `৳${Math.round(v).toLocaleString('en-IN')} / year`);
