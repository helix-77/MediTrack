...
- **`lib/screens/ai_assistant_screen.dart`** (persistent tab, can't be popped):
  ...
  - Replaced the fake "5 requests/day" banner with an accurate "MediTrack AI is a Premium feature (৳2.78/day)" CTA for free users, plus a live quota readout (`entitlement.checkAiQuota().statusMessage`) for already-subscribed users.
